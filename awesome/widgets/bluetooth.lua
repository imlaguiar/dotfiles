...
local function draw_icon(surface, state, connected)
   ...
   cr:set_line_width(8)
   cr:set_source(gears.color(beautiful.fg_occupied))
   -- if connected then
...
function bluetooth.new()
   ...
   local function set_entries_size(entries)
      ...
      entries.theme = { height = 24, width = len, fg_normal = beautiful.fg_normal,
   ...
   awful.key({ modkey }, "b", function() menu_fn(true) end, { description = "Toggle bluetooth menu", group = "launcher" })
   ...
   awesome.connect_signal("toggle_bluetooth_signal", function() menu_fn() end)
